...
- **Skills Registry**:
  - `bug-hunter` (v1.0.0) — Supreme autonomous skill that performs a comprehensive, multi-agent codebase sweep to identify concrete bugs (correctness, concurrency, monetary, logic, memory, and security flaws). Uses an adversarial verification process via sub-agents to refute false positives before generating a highly detailed, actionable markdown report.
